...
DBH_DS = DBHDataSet_train_aug(root = "/home/scion.local/davidsos/Documents/Grove_Regression/train_edges_chance_302_0", 
                              transform = fly_transform) # , , transform = fly_transform
...
DBH_DS_val = DBHDataSet_val(root = "/home/scion.local/davidsos/Documents/Grove_Regression/val_edges_chance_302_0") # , 
...
DBH_DS_test = DBHDataSet_test(root = "/home/scion.local/davidsos/Documents/Grove_Regression/test_edges_chance_302_0") #, 
...
#         self.dropout60 = torch.nn.Dropout(0.5)
        self.regressor = Linear(FILTERS, 1)  # 32, 1 # in_features, out_features
...
#         row, col = knn(pos, pos[idx], k=K, batch, batch[idx]) # , loop=LOOP)
...
#         h = F.dropout(h, p=0.1, training = self.training)
        ...
        # 4. Global Pooling.
        h = global_max_pool(h, batch)  # pools the features [num_examples, hidden_channels]
#         h = global_sort_pool(h, batch, k=32)
#         h = Conv1d(h, 32, 3) # in_channels, out_channels, kernel_size, stride=1,
        # 5. Classifier.
...
criterion_hub = torch.nn.HuberLoss(reduction='mean')
...
def train(model, optimizer, loader):
    ...
    for data in loader:
        ...
        loss = criterion_mse(logits.to(torch.float32).squeeze(-1).to(device), data.chance.to(torch.float32).to(device)) # Loss computation. # logits[1]
...
def test_rmse(model, loader):
    ...
    for data in loader:
        preds = model(data.pos.to(device), data.edge_index.to(device), data.batch.to(device))
#         y_output_pred.squeeze(-1)
        loss = criterion_mse(preds.to(torch.float32).squeeze(-1).to(device), data.chance.to(torch.float32).to(device)) # Loss computation.
        total_MSE += loss.item() #  * data.num_graphs
        count+=1
...
for epoch in range(1, 100000):
    ...
    test_HUB = test_hub(model, test_loader)
    ...
    if val_mse < best_val_mse:
        ...
        mod_name = 'grove_chance_edge_index_302_3.pt' #  + "_" + str(epoch) + "_" +  str(best_val_mse) + '.pt'
        torch.save(model.state_dict(), mod_name) # 'model_inMemory_tr_test_minmax_noAug_K16_v2.pt'
        print(f'Epoch: {epoch:02d}, TR: {loss:.4f}, VAL: {val_mse:.4f}, TEST: {test_mse:.4f}, TEST_HUB: {test_HUB:.4f}')
        print("saving", mod_name)
    else:
        print(f'Epoch: {epoch:02d}, TR: {loss:.4f}, VAL: {val_mse:.4f}, TEST_MSE: {test_mse:.4f}, TEST_HUB: {test_HUB:.4f}')
...
model_path = 'grove_chance_edge_index_302_3.pt' # '/home/scion.local/davidsos/Documents/DBH_Regression/model_19032022_StdS_conv3_dropout_10000_60_valAUG_MAE1.pt'
...
#         count+=1
    ...
    return DATA.chance.item(), pred.to(torch.float32).squeeze(-1).to(device).item() # len(test_loader.dataset))
